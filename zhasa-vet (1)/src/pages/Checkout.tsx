...
import { ShoppingBag, MapPin, User, CheckCircle2, ArrowLeft } from 'lucide-react';
      ...
      <div className="flex flex-col lg:flex-row gap-16">
        ...
        <div className="flex-grow space-y-12">
          ...
          <form className="space-y-10" onSubmit={handleSubmit}>
            ...
            >
              Подтвердить заказ на {total.toLocaleString()} ₸
            </button>
          </form>
        </div>
            ...
            <div className="space-y-6 max-h-[40vh] overflow-y-auto pr-4 custom-scrollbar">
              {cart.map(item => (
                <div key={item.id} className="flex gap-4">
                  <img src={item.image_url} alt="" className="w-16 h-16 rounded-2xl object-cover" />
                  <div className="flex-grow space-y-1">
                    <p className="font-bold text-slate-800 text-sm leading-tight line-clamp-2">{item.title}</p>
