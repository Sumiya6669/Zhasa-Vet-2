...
import { ShoppingCart, Menu, X, Phone, MapPin, Instagram, Facebook } from 'lucide-react';
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 glass">
      <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 group">
          <div className="w-10 h-10 bg-brand-teal rounded-xl flex items-center justify-center text-white font-display font-bold text-xl transition-transform group-hover:scale-110">
            Z
          </div>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-20 left-0 right-0 bg-white border-b shadow-lg md:hidden py-4 px-4 flex flex-col gap-4"
          >
            {links.map(link => (
              <Link 
                key={link.path} 
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
        <div className="space-y-4">
          <Link to="/" className="flex items-center gap-2">
            <div className="w-8 h-8 bg-brand-teal rounded-lg flex items-center justify-center text-white font-display font-bold text-lg">
              Z
            </div>
